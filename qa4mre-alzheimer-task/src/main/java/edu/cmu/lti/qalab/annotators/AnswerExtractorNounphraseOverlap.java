...
public class AnswerExtractorNounphraseOverlap extends JCasAnnotator_ImplBase {
	...
	public void process(JCas aJCas) throws AnalysisEngineProcessException {
		...
		for (int i = 0; i < qaSet.size(); i++) {
			...
			int topK = Math.min(3, candSentList.size());
			int selectedIdx = 0;
			int correctIdx = 0;
			String correct = "";
			for (int c = 0; c < topK; c++) {
				...
				for (int j = 0; j < choiceList.size(); j++) {
					...
					CandidateAnswer candAns = hshAnswer.get(choiceList.get(j)
							.getText());
					if (candAns == null) {
			...
			try {
				...
				if (bestChoice != null) {
					...
					choiceList.get(bestChoice.getChoiceIndex()).setIsSelected(
							true);
				}
			} catch (Exception e) {
				e.printStackTrace();
